...
const getFileName = filePath => {
  ...
  // Returns: '.html'
  const ext = path.extname(filePath); // path.basename('/foo/bar/baz/asdf/quux.html', '.html');
  // Returns: 'quux'
...
class AutoExport {
  ...
  getFile(path) {
    ...
    path.map((item, index) => {
      fs.stat(item, (firsterr, firstData) => {
        // 判断是否是文件夹
        const isDirectory1 = firstData.isDirectory()
        if(isDirectory1) {
  ...
  handleChange(isDelete = false) {
    // pathname：https://www.jianshu.com/p/e7b530579ab3
    return (pathname, stats) => {
      if (!this.compileHasError) {
        // 运行环境目录下
        console.log('\x1b[34m auto export compiling ... \x1b[0m');
        const absolutePath = path.resolve(pathname); // absolutePath = /Users/yfm/Downloads/react-project/auto-export-plugin/constant/index.js
        // 使用test.js测试 pathname = constant/test.js
        const ast = this.getAst(absolutePath); // 1:path
        this.handleAst(ast)
      }
    };
  }
  ...
  handleIndexChange(changedFilePath, isDelete) {
    ...
    if (watchDirs.includes(dirName)) {
      ...
      // 比如this.options.dir = ['constant', 'src'],
      // 变化的文件为constant/index.js， 则不再向constant的上级目录写入
  ...
  getExportNames(filename, defaultName) {
    // 1:path 2:name
    const ast = this.getAst(filename); // 1:path

    let exportNameMap = {};

    try {
      traverse(ast, {
        // ExportNamedDeclaration是钩子函数，可在babel-types查找对应的API，其他类似
        // 主要处理export const a = 1这种写法
        ExportNamedDeclaration(path) {
          // 考虑到一个文件中可能变量声明语法较多但不一定是export，比如
          // export function getOne(){} export const a = 1
          // 所以对于`export const a = 1`这种写法，
          // 没有采用像其他3种方式一样单独对类型做处理，
          // 而是在ExportNamedDeclaration中进一步做判断并处理
  ...
  autoWriteFile(writeFilePath, changedFileName, nameMap, isReplace) {
    if (!isReplace) {
      ...
      // 1.删除了文件
      // 2.文件没有导出任何内容，但是还是要去index.js文件中去遍历一下，
      // 看是否之前已经写入了该文件的导出语句， 如果有应该删除掉
    ...
    traverse(ast, {
      ...
      ImportDeclaration(path) {
        ...
        if (path.node.source.value === relPath) {
          // 如果存在import xxx, { xxx } from relPath, 
          // 把旧的变量收集起来并且检测export语句把这些变量删除。 
          // 同时新增export { xx } from relPath
      ...
      ExportSpecifier(path) {
        if (!_.isEmpty(oldImportNames) && oldImportNames.includes(path.node.exported.name)) {
          ...
          path.remove(); //进一步判断是否还有其他语句导出， 如果没有移除该条语句， 防止export {}导出空对象
